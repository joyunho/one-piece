...
function finalStageSnapshot(state,counts,mode,route,settings,fixed){
  // v25.0.1: 여기는 '실제 보드' 스냅샷이다 — 반대 계통 유닛도 보드 위에서
...
// v25.0.1(사용자 리포트): 나미(마딜) 참고 파티에 보유 사보(히든 [물딜])가
...
function mainUpperFor(state,counts,fixed,mode){
  ...
  // v25.0.1: 계획 경로에서는 mode 를 넘겨 반대 계통 보유 상위가 메인으로
...
function planFinalSquad(input){
  ...
  // v25.0.1: 계통 게이트가 계획에서 뺀 보유 최종 유닛을 근거로 노출한다 —
