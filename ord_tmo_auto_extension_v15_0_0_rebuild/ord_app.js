...
class App{
  ...
  authoritativeDecision(settings){
    ...
    if(key!==this._v15CacheKey){
      ...
      const decideSettings=Object.assign({},settings,{_blueprintRankings:this._blueprintRankingsKey===key?this._blueprintRankings:null,_blueprintPlanSync:!workerReady,_stickyActionId:this._stickyActionId||'',_craftLockId:this.v202CraftLockId(settings)});
  ...
  // v20.2(사용자 요청 "완성도 %가 안보이니까 불편한데"): 보강이 없을 때
  // '보강 없음'만 띄우면 진행도를 볼 방법이 아예 없다.  그렇다고 TMO 값을
  // 흉내내면 거짓이 되므로, 코치가 자기 원장(recipeSolve 흔함 잔여)으로
  // 직접 잰 값을 **다른 이름으로** 보여 준다.  두 수치는 같지 않다.
  v202Completion(state,unitOrId){
    const unit=unitOrId&&unitOrId.id?unitOrId:state&&state.db&&state.db.byId.get(String(unitOrId||'')),id=String(unit&&unit.id||unitOrId||'');
    // state.percent 는 TMO 보강이 실려야 생긴다 — 없으면 TMO 값을 읽을
    // 대상 자체가 없으므로 보강 판정과 무관하게 원장 계산으로 간다.
    if(this.v1912TmoEnriched()&&unit&&state&&state.percent)
      return{percent:C.num(C.completionPercent(state,unit)),source:'tmo',label:'TMO 완성도',short:'TMO'};
    const ledger=state&&state.db?C.ledgerCompletion(state.db,id,state.counts||{}):null;
    if(!ledger)return{percent:0,source:'none',label:'완성도 확인 불가',short:'—',note:'재료 원장을 읽지 못했습니다'};
    return{percent:C.num(ledger.percent),source:'coach',label:'코치 계산 재료 진행도',short:'코치',owned:!!ledger.owned,
      needTotal:C.num(ledger.needTotal),needRemain:C.num(ledger.needRemain),
      note:ledger.owned?'이미 보유':`흔함 환산 ${C.num(ledger.needTotal)-C.num(ledger.needRemain)}/${C.num(ledger.needTotal)}장 · TMO 수치와 다름`};
  }
  ...
  renderV151NextAction(state,plan,health){
    ...
    return`<div class="v151-action ${C.esc(status.toLowerCase())}" data-state="${C.esc(status)}">${status==='HOLD'?'<div class="v1915-hold-banner">이 카드는 지금 만들라는 추천이 아닙니다 — 필수 역할을 지키기 위해 <b>보류</b>된 후보입니다. 사유와 회복 목표를 먼저 확인하세요.</div>':''}${(()=>{
      ...
      const done=this.v202Completion(state,unit);
      return done.source==='none'?'':`<div class="v151-cost v202-progress ${done.source}"><small>완성도</small><b>${Math.round(C.num(done.percent))}<i>%</i></b><span>${C.esc(done.source==='coach'?'코치 원장 계산':'TMO 보고값')}</span></div>`;
    })():''}</div>${deltas.length?`<div class="v151-deltas">${deltas.map(row=>{
  ...
  v1912MidJoinBanner(){
    ...
    return`<div class="v159-endgame v1912-midjoin">${this.v153Icon('warn')}<span><b>중간 합류 감지</b> — 게임 시작(패 0→1)을 보지 못해 1라부터 세는 중일 수 있습니다. 상단 −1/+1로 <b>실제 라운드</b>를 맞춰주세요. 보스 예고·상위 확정(25라)·마감 계산이 라운드에 걸려 있습니다.</span></div>`;
  }
  ...
  renderV153Preview(state,plan){
    ...
    const queueHtml=queue.length?queue.map((item,index)=>`<button class="v158-queue-row${item.goal?' v202-goal-row':''}" data-act="detail" data-id="${C.esc(item.id)}"><i class="v1910-flex" title="${item.goal?'앞으로 닫을 목표 · 지금 만들라는 뜻이 아님':'순서 없는 가변 후보'}">${item.goal?'목표':'—'}</i>${item.unit&&item.unit.image?`<img src="${C.esc(item.unit.image)}" alt="">`:this.v153Icon('placeholder')}<span><b>${C.esc(item.name)}</b><small>${C.esc(item.note)}</small></span><em>${this.v153Icon('spiral')}${C.num(item.wispCost)}${(()=>{
      // v20.2: 큐 행에도 완성도를 단다 — "몇 % 남았나"를 보려고 매번 상세를
      // 열지 않게.  근거(TMO/코치)는 색으로 갈린다.
      const done=this.v202Completion(state,item.id);
      return done.source==='none'?'':`<b class="v202-queue-pct ${done.source}" title="${C.esc(done.label)}">${Math.round(C.num(done.percent))}%</b>`;
    })()}</em></button>`).join('')
    ...
    return`<div class="v158-queue" data-state="${C.esc(stateKey)}">${queueHtml}</div><p class="v154-next-note">확정은 왼쪽 큰 카드 하나뿐 — 아래 목록은 순서 없는 가변 후보이며 패가 바뀌면 다시 계산합니다. <b>목표</b> 표시는 지금 만들라는 뜻이 아니라 그 역할을 닫을 후보입니다.</p>`;
  }
  ...
  renderV153CraftableLegends(state,plan){
    ...
    // 문구('희귀 n/m')와 같은 보유 비율을 그린다(8-2) — TMO 완성도는
    // 우상단 배지·푸터 숫자로만.
    ...
    const renderCard=(row,index)=>{
      ...
      // v20.2: TMO 보강이 없으면(데스크톱 셸) 그 값은 항상 0이라 아무것도 못
      // 보여 줬다.  그때는 코치 원장 진행도로 대체한다(이름·근거를 따로 표기).
      const done=this.v202Completion(state,row.unit),tmoPct=Math.round(C.num(done.percent));
      ...
      const ratioHtml=row.upcoming||done.source==='none'?'':`<em class="v156-ratio ${row.feasible?'ok':''} ${done.source==='coach'?'v202-coach-pct':''}" title="${C.esc(done.label)}">${tmoPct}<i>%</i></em>`;
      return`<button class="${recommended?'recommended':''} ${row.upcoming?'upcoming':row.feasible?'ready':'waiting'}" data-act="detail" data-id="${C.esc(row.unit.id)}">${recommended?'<i class="v153-pick">추천</i>':row.upcoming?'<i class="v153-pick dim">노리기</i>':''}<header>${row.unit.image?`<img src="${C.esc(row.unit.image)}" alt="">`:this.v153Icon('placeholder')}<span><b>${C.esc(displayNameOf(row.unit))}</b><small>${C.esc(row.roles||'역할 보조')}</small></span>${ratioHtml}</header><div class="v154-rare-progress"><strong>희귀 ${owned}/${total}</strong><em>${C.esc(status)}</em></div><i class="v154-rare-bar${row.feasible?' full':''}"><span style="width:${total>0?Math.round(owned/total*100):0}%"></span></i>${recipeLine}${usedLine}${coversHtml}<div class="v154-rare-mats">${ingredients}</div><footer>${this.v153Icon('spiral')}<b>선위 ${C.num(row.solve&&row.solve.wispCost)}</b><span>${done.source==='none'?'':`${C.esc(done.label)} ${tmoPct}% · `}${C.num(progress.short)}장 남음${done.source==='coach'?' · TMO 보강 없음(코치 원장 계산)':''}</span></footer></button>`;
    };
  ...
  renderDetail(state,plan){
    ...
    return`<div class="modal-back" data-act="close-detail"><article class="detail-modal" role="dialog" aria-modal="true" aria-label="${C.esc(displayNameOf(u))} 상세"><button class="modal-x" data-act="close-detail" aria-label="닫기">×</button><header><img src="${C.esc(u.image||'')}" alt=""><div><h2>${C.esc(displayNameOf(u))}</h2><p>${C.esc(tierLabel(u))} · ${(()=>{const done=this.v202Completion(state,u);return done.source==='none'?'완성도 확인 불가':`${C.esc(done.label)} ${pct(done.percent)}`;})()}</p>${storyBadge(u,story)}</div></header><div class="detail-hero"><div><small>${C.esc(wisp.basisLabel)} 필요 선택위습</small><b>${wisp.planned}</b><span>가용 ${wisp.available} · 제작 후 ${wisp.after}${wisp.different?` · 현재 패 단독 ${wisp.current}`:''}</span></div><div><small>판정</small><b>${C.esc(status)}</b><span>${C.esc(C.summarizeRoles({role},plan.mode))}</span></div><div class="story-detail"><small>스토리 등급 · ${C.esc(story.basisLabel)}</small><b>${C.esc(story.label)}</b><span>${C.esc(story.note)}</span></div><div class="detail-command"><small>조합 명령어</small><span><i>한글</i><b>${C.esc(command.koreanDisplay)}</b></span><span><i>English</i><b>${C.esc(command.englishDisplay)}</b></span>${command.inherited?`<em>원형 ${C.esc(command.sourceName)} 최초 제작 명령</em>`:''}</div></div><section class="why-detail"><h3>왜 이 유닛을 만드나</h3><p>${C.esc(why.headline||'제작 후 순증 스펙과 재료 효율을 확인하세요.')}</p>${impactHtml?`<div class="impact-grid">${impactHtml}</div>`:'<small>현재 클리어 조건의 직접 순증이 없습니다.</small>'}${why.alternative?`<div class="cheaper-alt">더 싼 대안: <b>${C.esc(displayNameOf(why.alternative.unit))}</b> · 선위 ${why.alternative.wispCost}</div>`:''}${why.tradeoffs&&why.tradeoffs.length?`<div class="trade-warning">소모 재료 영향: ${C.esc(why.tradeoffs.join(' · '))}</div>`:''}</section>${(()=>{
