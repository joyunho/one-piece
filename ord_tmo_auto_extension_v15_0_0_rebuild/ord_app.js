...
class App{
  ...
  authoritativeDecision(settings){
    ...
    const strategic={
      round:C.num(settings.currentRound),mode:settings.mode||'',magicRoute:settings.magicRoute||'',postLegendRoute:settings.postLegendRoute||'',gorosei:settings.gorosei||'none',story10Reward:settings.story10Reward||'',vetoIds:(this.state.vetoIds||[]).join(','),
  ...
  renderV22PhasePanel(state,plan){
    ...
    if(ph.key==='p2')return`${goal(milestone!=='firstRare'&&milestone!=='firstFinal','20라 보스 전 전설급 1')}${milestone==='firstRare'?goal(false,'첫 희귀가 아직입니다 — 이것부터'):''}${goal(false,'S급이 +5선위 안이면 투자, 아니면 최저 선위')}${this.v221UpperMaterialsBlock(state)}${this.v221StoryBlock()}${fullSpec}`;
    if(ph.key==='p3'){
