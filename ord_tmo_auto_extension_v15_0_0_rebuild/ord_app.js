...
const DEFAULTS={
  tab:'coach',mode:'',magicRoute:'auto',settingsRevision:178,modeExplicit:false,targetSquadCount:9,purpose:'',gorosei:'none',superKumaOwned:true,story10Reward:'',virtualSpecialId:'',virtualSpecialBaselineId:'',virtualSpecialBaselineCount:0,wispOverride:'',upperPreviewId:'',upperBlueprint:null,
...
class App{
  ...
  authoritativeDecision(settings){
    ...
    const strategic={
      round:C.num(settings.currentRound),mode:settings.mode||'',magicRoute:settings.magicRoute||'',postLegendRoute:settings.postLegendRoute||'',gorosei:settings.gorosei||'none',story10Reward:settings.story10Reward||'',
    ...
    if(key!==this._v15CacheKey){
      ...
      catch(error){this._v15Cache={version:'17.12.0',authority:true,state:'SYNC_BLOCKED',label:'판단 엔진 점검 필요',reason:String(error&&error.message||error),action:null,alternatives:[],unknowns:['판단 엔진 오류']};}
    ...
    const pending=this.state.pendingReroll;if(pending)return Object.assign({},base,{state:'SYNC_BLOCKED',label:'리롤 결과 확인 대기',reason:`${pending.name||'희귀'} 1장 리롤 뒤 해당 희귀 수량이 감소한 TMO 패를 기다립니다. 다른 패 변화만으로는 다음 리롤을 열지 않습니다.`,action:null,blockedAction:null,pendingKind:'reroll'});
    return base;
  }
  plan(){
    ...
    if(!global.ORDV15Engine&&planner&&typeof planner.rankDeckDirections==='function'&&plan.purpose==='upper'&&!upperLocked&&!postLegend.awaiting){
      const manual=Object.entries(settings.manualCounts||{}).sort().map(pair=>pair.join(':')).join(','),rankInputs={manual,virtualSpecialId:settings.virtualSpecialId,virtualSpecialBaselineId:settings.virtualSpecialBaselineId,virtualSpecialBaselineCount:settings.virtualSpecialBaselineCount,wispOverride:settings.wispOverride,gorosei:settings.gorosei,superKumaOwned:settings.superKumaOwned,changedUsed:settings.changedUsed,seraphUsed:settings.seraphUsed,transcendUsed:settings.transcendUsed,roundBand:settings.currentRound<50?'pre50':'50plus'},rankKey=[fingerprint(this.state.snapshot),JSON.stringify(rankInputs)].join('|');
  ...
  setOpt(key,val){const before=this.state[key];if(key==='mode'){this.selectDamageMode(val);return;}if(['currentRound','roundPrepSeconds','roundNormalSeconds','roundBossSeconds','transcendUsed','seraphUsed','changedUsed'].includes(key))val=Math.max(0,C.num(val));if(key==='wispOverride'&&this.state.pendingTransaction)this.rollbackTransaction();if(key==='virtualSpecialId'){val=String(val||'');if(val!==String(before||'')){this.state.virtualSpecialBaselineId=val;this.state.virtualSpecialBaselineCount=val?rawSnapshotCount(this.state.snapshot,val):0;}else if(!val){this.state.virtualSpecialBaselineId='';this.state.virtualSpecialBaselineCount=0;}}this.state[key]=val;if(['wispOverride','virtualSpecialId'].includes(key))this.releaseDirectionHold();if(key==='virtualSpecialId'){this._upperRankCacheKey='';this._upperRankCache=[];}this._squadCacheKey='';this._deferredExternalRender=false;if(RUN_LOG_SETTING_KEYS.has(key))this.recordAuditAction({actor:'user',action:'setting-change',key:String(key),before:before===''?null:before,after:val===''?null:val});this.persist();this.render();}
  commandInfo(unit){
  ...
  act(a,b){
    ...
    if(a==='reroll-confirmed'){
      // v17.6(감사 P0-1): 게임당 리롤 총 2회 — 엔진이 이미 차단하지만
      // UI에서도 이중으로 막는다(엔진 캐시 지연 대비).
      if(C.num(this.state.rerollsUsed)>=2){this.toast('희귀 리롤은 게임당 2회입니다 — 이미 모두 사용했습니다.');return;}
      ...
      this.state.rerollsUsed=Math.min(2,C.num(this.state.rerollsUsed)+1);this.state.pendingReroll={id:rare.id,name:rare.name,baseFingerprint:fingerprint(this.state.snapshot),beforeCount:C.num(this.state.snapshot&&this.state.snapshot.counts&&this.state.snapshot.counts[rare.id]),baseSeq:C.num(this.state.snapshot&&this.state.snapshot.seq),at:Date.now()};this._v15CacheKey='';this.recordAuditAction({actor:'user',action:'rare-reroll-confirmed',targetId:rare.id,targetName:rare.name,count:1,proof:rare.proof||null});this.persist();if(this.onConnectionTest)this.onConnectionTest();this.toast(`${rare.name} 1장 리롤 결과를 기다립니다. 해당 희귀 수량이 줄기 전에는 추가 리롤을 잠급니다.`);return;
    }
  ...
  renderUnsafe(){
    ...
    this.root.innerHTML=`<div class="ord-app v151-shell" data-view="${C.esc(this.state.tab)}">${body}${this.renderDetail(state,plan)}${this.renderV151PartyModal(state,plan)}${this.renderV151ShipModal(state)}${this.renderRunResultModal(health)}${this.state.message?`<div class="ord-toast" role="status">${C.esc(this.state.message)}</div>`:''}</div>`;
  ...
  v151ShipPlan(state){
    ...
    const mode=(this.state&&this.state.mode)||this.v151FamilyIntent(state)||'';
    const rows=products.map(unit=>{
      ...
      return{unit,kind,family,familyFit,feasible:!missing.length,missing,missingRares:missing.filter(m=>m.rare)};
    });
    const order=(a,b)=>Number(b.familyFit)-Number(a.familyFit)||Number(b.feasible)-Number(a.feasible)||a.missing.length-b.missing.length||C.nameOf(a.unit).localeCompare(C.nameOf(b.unit),'ko');
  ...
  renderV151Preparation(state,plan){
    ...
    // v17.11(사용자 요청): 리롤로 노릴 희귀 목표 + 적중 확률.
    ...
    // v17.12(사용자 요청 2): 해적선 전체 목록은 2번 패널을 어지럽혔다 —
    // 여기는 추천 1개 한 줄만 남기고, 전체 비교는 "전체" 모달로 뺀다.
    ...
    return items.length||rerollHint||shipHint||targetsHtml?`<div class="v151-prep-list">${items.slice(0,5).map((item,index)=>{const unit=state.db&&state.db.byId.get(item.id);return`<button data-act="detail" data-id="${C.esc(item.id)}">${unit&&unit.image?`<img src="${C.esc(unit.image)}" alt="">`:`<i>${index+1}</i>`}<span class="v151-prep-copy"><span class="v151-prep-name"><b>${C.esc(item.name)}</b>${this.v151StoryTag(unit)}</span><small>${C.esc(item.note)}</small></span><em>선위 ${C.num(item.wispCost)}</em></button>`;}).join('')}${targetsHtml}${shipHint}${rerollHint}<p>미리 준비만 하며, 실제 다음 행동은 TMO 패 변화 뒤 다시 확정합니다.</p></div>`:`<div class="v151-empty"><b>현재 행동만 확정</b><span>다음 행동은 지금 미리 고정하지 않고 TMO 변화 뒤 계산합니다.</span></div>`;
  }
  ...
  renderV151CurrentSpec(state,plan){
    ...
    return`<div class="v151-spec-head"><div class="damage-mode-switch" role="group" aria-label="딜 계통 선택"><button class="${!this.state.mode?'on':''}" data-act="mode" data-value="">자동</button><button class="${this.state.mode==='physical'?'on':''}" data-act="mode" data-value="physical">물딜</button><button class="${magic?'on':''}" data-act="mode" data-value="magic">마딜</button></div>${magic?`<select data-opt="magicRoute" aria-label="마딜 경로"><option value="auto" ${route==='auto'?'selected':''}>자동 경로</option><option value="dual" ${route==='dual'?'selected':''}>2상위·토키</option><option value="singleEnd" ${route==='singleEnd'?'selected':''}>1상위·단끝</option></select>`:''}${resolvedChip}<strong>선위 ${C.num(state.wisp)}</strong></div>${(()=>{const lab=this.state.labResearch||{},box=(key,label)=>`<label><input type="checkbox" data-upg="${key}" ${lab[key]?'checked':''}><span>${label}</span></label>`;return`<div class="v151-upg-row"><small>연구소 <i>풀 강화 가정 · 다르면 해제</i></small>${box('attack','공업+12%')}${box('slow','이감+10')}${box('hpRegen','체젠+0.45')}${box('mpRegen','마젠+0.8')}<label class="v151-upg-level"><span>등급 공업 Lv</span><input type="number" min="1" max="21" data-upg="upperLevel" value="${C.num(this.state.upperResearchLevel)||1}"></label></div>`;})()}${summary}<div class="v151-spec-body">${body}</div>`;
  }
  ...
  v151ClearWhy(state,plan,row){
    ...
    const cacheKey=`${row.id}|${row.routeKey||''}`;
  ...
  v151RerollTargets(state,plan,decision){
    ...
    const kinds=targets.size,rerollLeft=Math.max(0,2-C.num(this.state.rerollsUsed));
  ...
  renderV151UpperInfo(state,plan){
    ...
    return`${main}${lockedPath}${dpsLine}${needs}${cards?`<div class="v151-upper-candidates">${cards}</div>`:`<div class="v151-upper-note">${upper?'확정 상위 중심으로 다음 행동을 계속 계산합니다. 패가 바뀔 때마다 아래 필수 역할 충족을 다시 검사합니다.':'상위 선택 단계가 되면 최대 6개 후보가 여기에 표시됩니다.'}</div>`}${gatedHtml}`;
  }
