...
3. 이후에는 바탕화면 바로가기만 실행하면 됩니다.
   (TMO.GG 데스크톱 프로그램 먼저 → 코치 → 게임. F5 = 오버레이)
...
| 오버레이 | 불가 | **F5 — 인게임 HUD(투명·클릭 통과) · F6 — 미니 패널(위치 잡기) · F10 — 다음 모니터로 이동** |
| ordlog 저장 | 수동 내보내기 | **60초 자동 저장** (`문서/ORD_coach_logs/`) |
| 완성도% | TMO 도우미 계산값 | **코치 원장 계산값**(v20.2 — 라벨로 구분) |
| 현재 능력치 | TMO 탭 보강 | 없음(화면에 '보강 대기'로 표시) |
...
- 현재 능력치는 TMO 도우미 화면이 계산하는 값이라 셸에는 없습니다 —
  수량·위습·자동 라운드·추천은 전부 정상입니다.
- 완성도%는 v20.2 부터 코치가 직접 계산합니다(조합식을 바닥 흔함까지
  전개해 `1 − 현재잔여/맨손잔여`).  TMO 의 수치와 **같지 않으므로**
  화면에서 "코치 계산 재료 진행도"로 따로 이름 붙이고 색도 다르게
  칠합니다(계산=푸른색 / 관측=초록색).
